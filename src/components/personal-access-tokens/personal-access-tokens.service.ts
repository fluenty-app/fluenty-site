...
export class PersonalAccessTokensService {
  ...
  async createToken(user) {
    ...
    const token = await this.personalAccessTokenRepository.save({
      user: user,
      name: "",
  ...
  async impersonateToken(token: PersonalAccessToken, impersonateUser: User) {
    if (token.userId === impersonateUser.id) {
      return false;
    }

    return this.personalAccessTokenRepository.setTokenImpersonation(token, impersonateUser.id)
  }
}
